...
#define LOG(fmt, ...) __android_log_print(ANDROID_LOG_INFO, "HPROF_STRIP", fmt, ##__VA_ARGS__)
...
//int open_real_proxy_auto(const char *pathname, int flags, mode_t modes) {
//  LOG(">>>>> open_real_proxy_auto");
...
ssize_t HookWrite(int fd, const void *buf, size_t count) {
//  LOG(">>>>> HprofStrip::HookWrite");
...
//  } else {
//    LOG(">>>>> hooked. stub: %x"
//        ", status: %d, caller_path_name: %s, sym_name: %s, new_func: %x, prev_func: %x"
//        ", arg: %x",
...
//
//  void *open_proxy, *write_proxy;
//  open_proxy = (void *)(open_real_proxy_auto);
//  write_proxy = (void *)(HookWrite);
//
//  bytehook_stub_t res=  bytehook_hook_all("libart.so", "open", open_proxy, hooked_callback, nullptr);
//  LOG(">>>>> res =%p", res);
//  res=bytehook_hook_all("libbase.so", "__open_real", open_proxy, hooked_callback, nullptr);
//    LOG(">>>>> res =%p", res);
//    res=bytehook_hook_all("libartbase.so", "__open_2", open_proxy, hooked_callback, nullptr);
//    LOG(">>>>> res =%p", res);

//    bytehook_hook_single(nullptr, "libc.so", "write", write_proxy, hooked_callback, nullptr);
//    bytehook_hook_single(nullptr, "libart.so", "write", write_proxy, hooked_callback, nullptr);
//    bytehook_hook_single(nullptr, "libbase.so", "write", write_proxy, hooked_callback, nullptr);
//    bytehook_hook_single(nullptr, "libartbase.so", "write", write_proxy, hooked_callback, nullptr);
...
HprofStrip::HprofStrip()
  ...
  std::fill(strip_index_list_pair_,
            strip_index_list_pair_ + sizeof(strip_index_list_pair_)/sizeof(strip_index_list_pair_[0])/*arraysize(strip_index_list_pair_)*/, 0);
}
